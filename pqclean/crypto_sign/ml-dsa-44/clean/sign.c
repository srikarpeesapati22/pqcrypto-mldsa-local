...
/*************************************************
* Name:        PQCLEAN_MLDSA44_CLEAN_crypto_sign_keypair_with_seed
*
* Description: Generates public and private key when given a random seed.
*
* Arguments:   - uint8_t *pk:   pointer to output public key (allocated
*                               array of PQCLEAN_MLDSA44_CLEAN_CRYPTO_PUBLICKEYBYTES bytes)
*              - uint8_t *sk:   pointer to output private key (allocated
*                               array of PQCLEAN_MLDSA44_CLEAN_CRYPTO_SECRETKEYBYTES bytes)
*              - uint8_t *seed: pointer to input seed (allocated array of random bytes)
*
*
* Returns 0 (success)
**************************************************/
int PQCLEAN_MLDSA44_CLEAN_crypto_sign_keypair_with_seed(uint8_t *pk, uint8_t *sk, uint8_t *seed) {
    uint8_t seedbuf[2 * SEEDBYTES + CRHBYTES];
    uint8_t tr[TRBYTES];
    const uint8_t *rho, *rhoprime, *key;
    polyvecl mat[K];
    ...
    /* Get randomness for rho, rhoprime and key */
    //randombytes(seedbuf, SEEDBYTES);
    for (uint8_t *dst = seedbuf, *src = seed; dst < seedbuf + SEEDBYTES; *dst++ = *src++);
    seedbuf[SEEDBYTES + 0] = K;
    seedbuf[SEEDBYTES + 1] = L;
    shake256(seedbuf, 2 * SEEDBYTES + CRHBYTES, seedbuf, SEEDBYTES + 2);
    rho = seedbuf;
